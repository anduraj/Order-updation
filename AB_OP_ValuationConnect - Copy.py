...
def validate_dateTime(date_text): #function checks if due time is present in datetime string and appends 17:00:00 if no time is present #TFS default Time 5 PM
    try:
        ...
        return date_text
def updateATS(zadd,mainclient,orderid,due,adrs,typ,portId,clientId,price,portal,client):
                        ...
                        if zadd.__contains__('#') or zadd.__contains__('Union') or zadd.__contains__('UNION') or zadd.__contains__('APT') or zadd.__contains__('apt'):
                            ...
                            for i in addfolders:
                                ...
                                if trial==listadd and zip==zippro:
                                    old="Z:\\BPO\\"+year1+"//"+month+"//"+currentdate+"//"+mainclient
                                    new="Z:\\BPO\\"+year1+"//"+month+"//"+currentdate+"//"+mainclient
                                    os.rename(old,new)
                                    #("Folder renamed")
                                    break
...
def check(session, resp, mainclient, subclient, portal, cid, ats_client_id, ats_portal_id,username): ## username
        client=mainclient+' '+subclient
        #link contains the base link
        if 'You are logged in' in resp.text:
            url = "https://vendor.valuationconnect.com/Orders/PendingOrders?_search=false&nd=1686219126462&rows=-1&page=1&sidx=&sord="
            data = {
                '_search': 'false',
                'nd': '1593950169261',
                #'nd': '1686207331675',
            ...
            headers = {
                ...
                'Referer': 'https://vendor.valuationconnect.com/Orders/Dashboard',
            ...
            if details==[]:
                ...
                #(details)
                for o in details:
                    order_details=o['OrderInfo']
                    try:
                        #(order_details["OnHold"])
                        if order_details['OnHold']== False:
                            address= '{} {} {} {}'.format(order_details['SubjectPropertyAddress1'],order_details['SubjectPropertyCity'],order_details['SubjectPropertyState'],order_details['SubjectPropertyPostalCode'])
                            folderadd= '{}, {}, {} {}'.format(order_details['SubjectPropertyAddress1'],order_details['SubjectPropertyCity'],order_details['SubjectPropertyState'],order_details['SubjectPropertyPostalCode'])
                            #######################################################################
                ...
                ####################################################################
                            ...
                            AssetType= order_details['VendorProduct'].strip()
                            Fee=order_details['VendorFee']
                            duedate=order_details['DueDate']
                            ...
                            updateATS(zadd,mainclient,orderid,duedate,address,AssetType,ats_portal_id,ats_client_id,Fee,portal,client)
...
def Query_JSON(json_file_path):
    ...
    for value in filtered_data:
        for values in value['values']:
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(f"{subclient}-ValuationConnect")
            ...
            if credstatus == 'Active':                           
                    ...
                    try:
                                    ...
                                    headers={
                                                ...
                                                'authority': 'vendor.valuationconnect.com',
                                                'method': 'POST',
                                                ...
                                                'origin': 'https://vendor.valuationconnect.com',
                                                'pragma': 'no-cache',
                                                'referer': 'https://vendor.valuationconnect.com/Account/Login?ReturnUrl=%2fOrders%2fDashboard',
                                    ...
                                    loginurl='https://vendor.valuationconnect.com/Account/Login?ReturnUrl=%2fOrders%2fDashboard'
                                    url = 'https://vendor.valuationconnect.com/Orders/Dashboard'
                                    ...
                                    random_sleep_time = randint(900,1200) #180
